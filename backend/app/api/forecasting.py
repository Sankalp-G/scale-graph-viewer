...
try:
    import app.proto.forecast_pb2 as forecast_pb2
    import app.proto.forecast_pb2_grpc as forecast_pb2_grpc

    _proto_error: Optional[BaseException] = None
except Exception as exc:  # pragma: no cover - import guard
    forecast_pb2 = None
    forecast_pb2_grpc = None
    _proto_error = exc
...
DEFAULT_GRPC_SERVER = os.getenv("FORECAST_GRPC_SERVER", "10.24.24.28:50053")
DEFAULT_RETURN_EVERY_SECONDS = float(os.getenv("FORECAST_RETURN_EVERY_SECONDS", "5.0"))
DEFAULT_NUM_RECORDS = int(os.getenv("FORECAST_NUM_RECORDS", "100"))
DEFAULT_STREAM_IDS = [
...
def ensure_grpc_ready() -> None:
    if grpc is None or forecast_pb2 is None or forecast_pb2_grpc is None:
        ...
        raise HTTPException(
            ...
            detail=(
                ...
                "generate forecast_pb2.py and forecast_pb2_grpc.py under backend/. "
...
def choose_horizon(
    edge_results: Mapping[str, "forecast_pb2.EdgeResultList"],
    preferred: Optional[str],
) -> Tuple[Optional[str], Optional["forecast_pb2.EdgeResultList"]]:
    if not edge_results:
        return None, None
...
async def run_forecast_stream() -> None:
    while True:
        async with state_lock:
            ...
            num_records = stream_state["num_records"]
        ...
        if grpc is None or forecast_pb2 is None or forecast_pb2_grpc is None:
            logger.error("gRPC dependencies missing for forecasting.")
            await asyncio.sleep(1)
            continue

        try:
            await consume_forecast(
                server,
                stream_ids,
                num_records,
            ...
            logger.exception("Forecasting gRPC request failed.")
            await asyncio.sleep(1)


async def consume_forecast(
    server: str,
    stream_ids: List[str],
    num_records: int,
    ...
    request = forecast_pb2.InferenceRequest(
        stream_ids=stream_ids,
        num_records=num_records,
    )
    async with grpc.aio.insecure_channel(server) as channel:
        stub = forecast_pb2_grpc.InferenceServiceStub(channel)
        while True:
            async with state_lock:
                status = stream_state["status"]
            ...
            response = await stub.Run(request)
            selected_key, selected_results = choose_horizon(
                response.edge_results,
                horizon,
            )
            results_payload = []
            ...
            await asyncio.sleep(max(0.1, return_every_seconds))
...
async def start_stream(payload: Optional[StartRequest] = None) -> Dict[str, str]:
    ...
    async with state_lock:
        ...
        if payload:
            ...
            if payload.num_records is not None:
                stream_state["num_records"] = payload.num_records
            if payload.horizon is not None:
                stream_state["horizon"] = payload.horizon
        stream_state["request_id"] += 1
        logger.warning(
            "forecasting start: server=%s streams=%d",
            stream_state["server"],
